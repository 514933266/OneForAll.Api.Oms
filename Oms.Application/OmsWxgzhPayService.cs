...
namespace Oms.Application
{
    ...
    /// 微信小程序支付
    /// </summary>
    public class OmsWxgzhPayService : OmsBaseService, IOmsWxgzhPayService
    {
        ...
        public async Task<BaseMessage> CreateOrderAsync(OmsOrderForm form)
        {
            ...
            // 3. 微信下单
            var payData = await _wxOrderManager.CreateWxgzhAsync(setting, order);
            if (payData == null)
            {
            ...
            {
                msg.Data = new OmsWxgzhCreateOrderDto() { OrderId = order.Id, OrderNo = order.OrderNo };
            ...
            msg.Data = new OmsWxgzhCreateOrderDto() { OrderId = order.Id, OrderNo = order.OrderNo, PayState = order.PayState, PayData = payData };
